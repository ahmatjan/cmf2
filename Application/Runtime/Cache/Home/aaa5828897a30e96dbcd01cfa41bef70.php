...
<?php if(isset($jsFiles)): if(is_array($jsFiles)): $i = 0; $__LIST__ = $jsFiles;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$file): $mod = ($i % 2 );++$i;?><script type="text/javascript" src="<?php echo ($siteInfo['webFolder']); ?>static/js/<?php echo ($file); ?>"></script><?php endforeach; endif; else: echo "" ;endif; endif; ?>
	...
	<div class="rightDiv">
		...
		<input type="text" id="title" name="title" value="<?php echo ($info['title']); ?>" /><br />
		<label for="keywords">关键字</label>
		<input type="text" id="keywords" name="keywords" value="<?php echo ($info['keywords']); ?>" /><br />
		<label for="abstract">摘要</label>
		<input type="text" id="abstract" name="abstract" value="<?php echo ($info['contract']); ?>" /><br />
		<label for="template">模板</label>
		<select id="template" name="template">
		<option value="article">缺省模板</option>
		</select><br />
		<label for="audit">审核</label>
		<input type="radio" id="audit0" name="audit" value="0" />不审核
		...
		<textarea name="content" id="content" rows="8" cols="60"><?php echo ($info['content']); ?></textarea>
...
//]]></script>
		...
		<span>现有附件：</span>
		<?php if(is_array($oldAttachments)): $i = 0; $__LIST__ = $oldAttachments;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div><span><?php echo ($vo['name']); ?></span><input type="hidden" name="oldAttachments[]" value="<?php echo ($vo['id']); ?>" /><a href="javascript:void(0);">×</a></div><?php endforeach; endif; else: echo "" ;endif; ?>
		</div><?php endif; ?>
		<label for="attachment">附件</label><input type="button" id="addAttach" name="addAttach" value="添加一个附件" /><br />
		<div id="attachments">
			<div>
				<label>&nbsp;</label>
		...
		<label>&nbsp;</label><input type="button" class="button" id="addSub" value="确定" />
		</form>
	</div>
	</body>
</html>