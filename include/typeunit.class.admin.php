...
class TypeUnit
{
    ...
    function ListAllType($channel=0,$nowdir=0)
    {
		...
		$q = "SELECT a1.id,a1.typedir,a1.typename,a1.ispart,a1.sortrank,a1.ishidden, a2.mycount as chid, a.mycount FROM `#@__arctype` a1 LEFT JOIN (select reid, count(*) as mycount from `#@__arctype` group by reid) a2 on a2.reid=a1.id left join (select typeid, count(*) as mycount from `#@__archives` group by typeid) a ON a1.id=a.typeid WHERE a1.reid=0 order by a1.sortrank";
        $this->dsql->SetQuery($q);
        $this->dsql->Execute(0);
        while($row = $this->dsql->GetObject(0))
        {
            ...
            if($ispart==0)
            {
                ...
                if(empty($row->chid))echo "<a href='catalog_do.php?cid={$id}&dopost=listArchives'>内容</a>|";
                if(empty($row->mycount)) echo "<a href='catalog_add.php?id={$id}'>增加子类</a>|";
