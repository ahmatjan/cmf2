$(document).ready(function(){
	$('#addSub').click(function(){
		...
		$.post('/tour/Sys/auditoptionAdd', {name: encodeURIComponent(name), audit:$('#audit').val(), type: $('#type0').prop('checked')? 0 : 1}, function(data){
	...
	$('.modify').click(function(){
		if($(this).html() == '修改'){
			...
			secondTd.html('<select class="type"><option value="0">不分人</option><option value="1" ' + (secondTd.html() == '分人' ? 'checked':'') + '>分人</option></select>');
		}else{
			var btn = this;
			...
			$(this).attr('disabled', true);
			$.post('/tour/Sys/auditoptionModify', {name: encodeURIComponent(name), type:$(this).parent().parent().find('.type').val(), audit: $('#audit').val(), id:$(this).parent().find('input.id').val()}, function(data){
				$(btn).removeAttr('disabled');
				switch(data){
					case 'existed':
	...
	$('.delete').click(function(){
		if(window.confirm('您确定要删除这个内容吗？')){
			$.post('/tour/Sys/auditoptionDelete', {id: $(this).parent().find('input.id').val()}, function(data){
				switch(data){
					case 'used':
